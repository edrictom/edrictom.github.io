    ...
    return (
        <div className="min-w-full flex flex-col justify-center items-center">
            ...
            <form action="/api/form" method="POST" className="flex flex-col space-y-4 w-2/5">
                ...
                <textarea required placeholder="What's on your mind?" rows="8" className="mt-3 resize-none block w-full rounded-md text-black bg-gray-200 border-transparent focus:border-blue-500 focus:bg-gray-100 focus:ring-0 placeholder-gray-400"></textarea>
                <button type="submit" className="px-6 py-4 text-lg font-bold hover:ring bg-sky-600 hover:shadow-2xl active:bg-sky-800 transition-colors duration-75 rounded-xl">
                    Submit
                </button>
            </form>
        </div>
